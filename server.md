...
瑞高支付SDK提供电话卡支付，支付宝支付和银联等多种支付方式.
支付结果通知方式:

①支付成功: 
当游戏使用上述两种方式支付成功之后,瑞高服务器会发送一次通知到游戏服务器(NoticeUrl),通知游戏支付成功.游戏服务器收到瑞高发送的支付成功通知请求之后,需要立即输出一个“success”字符串,瑞高服务器接收到“success”字符串默认通知游戏服务器成功。（如果没有在订单发起通知中传递NoticeUrl，瑞高服务器将会通知游戏服务器提供的默认地址）
收到游戏服务器返回的“success”之后,支付跳转到支付成功界面,显示支付成功.同时页面上自动调用客户端支付通知接口通知客户端支付成功.<br>
支付成功时,瑞高服务器通知游戏服务器的相关链接和参数如下:
...
|EncString	|通知加密字符串其中EncString是 商户ID,请求编号,瑞高帐号,SessionKey,商户密钥五个字符串连接之后用MD5加密生成的字符串.EncString= MD5(MerId.Act.Uin.SessionKey.MerchantKey);|
请求地址为：http://mqyx.much3g.com/index.php/user_center

游戏服务器发出请求后，瑞高服务器会验证EncString是否有效,验证SessionKey是否有效,然后返回结果给游戏服务器。
瑞高服务器返回的参数如下：
